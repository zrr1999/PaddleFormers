    ...
    def on_optimizer_end(self, args, state, control, **kwargs):
        ...
        if _dump_dir:
            ...
            for _i, (_b, _u) in enumerate(zip(biases, usages)):
                _bb = _b.detach().astype("float32").numpy()
                _uu = _u.detach().astype("float32").numpy()
                _before.append(
                    {
                        "index": _i,
            ...
            _upd = update.detach().astype("float32").numpy()
            _payload = {
                ...
                "usages_mean": usages_mean.detach().astype("float32").numpy().reshape(-1).tolist(),

...
class Trainer:
    ...
    def training_pipeline_step(
        ...
        # MTP magic send: 发布本累加期的 input_ids 列表。
        # paddlefleet 的 mtp_embedding_layer 文档指明该列表应由 DistDataLoader 写入
        # （set_data: "DistDataLoader writes the full accumulation-period list"），
        # 但本仓库没有任何 set_data 调用者，所以 last-stage 的 mtp_embed 取不到 ids
        # （AssertionError: input_ids not in magic_send_dict）。这里在 _pp_data_buffer
        # 被消费前就地发布，语义与文档一致：列表长度 = gradient_accumulation_steps，
        # 消费端按 magic_count % len(list) 取。env gate 默认关闭。
        if os.environ.get("MODEL_REPRO_FORCE_MTP_MAGIC_SEND", "0") == "1":
            try:
                ...
                _ids = []
                for _mb in self._pp_data_buffer:
                    if isinstance(_mb, dict) and "input_ids" in _mb:
                        _ids.append(_mb["input_ids"])
                    elif isinstance(_mb, (list, tuple)) and _mb:
                        _ids.append(_mb[0])
                if _ids:
                    mtp_magic_instance.set_data({"input_ids": _ids})
            except (ImportError, ModuleNotFoundError):
                pass

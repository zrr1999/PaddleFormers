...
def apply_glm_moe_dsa_training_contract(model_config, training_args, model_args, data_args):
    ...
    # LlmMetaConfig.mtp_attributes does not include enable_mtp_magic_send, so
    # set_llm_config never copies the CLI/YAML field onto GLMMoEModelProvider
    # (TransformerConfig default False). Honour YAML/CLI then an env override.
    _magic = getattr(training_args, "enable_mtp_magic_send", None)
    _magic_env = os.environ.get("MODEL_REPRO_MTP_MAGIC_SEND")
    if _magic_env is not None:
        model_config.enable_mtp_magic_send = _magic_env == "1"
    elif _magic is not None:
        model_config.enable_mtp_magic_send = bool(_magic)
    print(
        "[MTP-MAGIC-SEND] model_config.enable_mtp_magic_send="
        f"{getattr(model_config, 'enable_mtp_magic_send', None)} "
        f"(cli={_magic!r} env={_magic_env!r})",
        flush=True,
    )
